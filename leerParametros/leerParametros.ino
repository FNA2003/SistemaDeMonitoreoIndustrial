...
PZEM004Tv30 pzems[] = {
  PZEM004Tv30 (2, 3, 0x00), // Rx y Tx de Arduino, cruzados para cada PZEM004T y, el último parámetro es la dirección usada en MODBUS... No la uso
  PZEM004Tv30 (4, 5, 0x00),
  PZEM004Tv30 (6, 7, 0x00)
};

// Parámetros y métodos
const char* medidas[]  = { "CORRIENTE", "POTENCIA", "ENERGIA", "VOLTAJE", "FRECUENCIA", "FACTOR DE POTENCIA" };
const char* unidades[] = { "A",         "W",        "kWh",     "V",       "Hz",         " " };

typedef float (PZEM004Tv30::*PZEMMethod)();
PZEMMethod metodos[] = {
	&PZEM004Tv30::current, &PZEM004Tv30::power,     &PZEM004Tv30::energy,
	&PZEM004Tv30::voltage, &PZEM004Tv30::frequency, &PZEM004Tv30::pf
};

// Enteros para recorrer las listas de parámetros eléctricos
uint8_t indice = 0;
const uint8_t cantidadParam = 6;
...
void mostrarLectura(uint8_t id) {
  ...
  lcd.clear(); // Borro lo que tuviese la pantalla anteriormente
	...
	lcd.setCursor(0, 0);
	lcd.print('['); lcd.print(medidas[id]); lcd.print(']');

  if (id == 1 || id == 2) { // POTENCIA ó ENERGÍA, se muestra la suma
    
    // Leo cada PZEM y voy sumando su lectura de potencia o energía
    for (auto& pzem : pzems)
      auxiliar += (pzem.*metodos[id])();
    ...
    lcd.setCursor(0, 3); lcd.print(auxiliar, 1); lcd.print(" "); lcd.print(unidades[id]);
    return;
  }
  ...
  for (uint8_t i = 0; i < cantidadSensores; i++) { // NOTA; Acá recorremos por índice para usar el número de 'i'
    ...
    lcd.print(auxiliar, 1); lcd.print(" "); lcd.print(unidades[id]);
  }
}
